...
const validUrl: string = `${homesOrigin}/machimusubi/sitemap-station.xml`;
...
describe('/sitemap-station.xml E2E', () => {
    ...
    test(`Check sitemap by machimusubi/{pref}/{station_name}-st`, async () => {
        ...
        const urlRegex = new RegExp(`${homesOrigin}/machimusubi/[A-Za-z]+/[A-Za-z]+_[0-9]+-st/`);
        //check format
